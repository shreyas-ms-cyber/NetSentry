"""
Traffic Routes - New implementation with guaranteed top_talkers
"""
...
def get_traffic():
    ...
    result = []
    for stat in stats:
        # Get top_talkers safely
        top_talkers = stat.top_talkers
        if top_talkers is None:
            top_talkers = []
        elif isinstance(top_talkers, str):
            try:
                top_talkers = json.loads(top_talkers)
            except:
                top_talkers = []
        ...
        item = {
            ...
            'top_talkers': top_talkers  # Explicitly add top_talkers
        }
        result.append(item)
